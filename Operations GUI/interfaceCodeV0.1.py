...
class operationsApp(Tk):
		...
		if self.systemClicked.get() == 0:
			...
			if self.operator.get() == "Enter Operator Name Here":
				...
				if self.fileOpened == False:
					...
					self.fileName = '/home/rossebv/Documents/RadMAP Tests/%s%s' %(operatorName,self.currentClock.strftime('%m%d%y'))
					if glob.glob("%s.txt" %self.fileName): 
						if glob.glob(('%sv*.txt' %self.fileName)):
							...
							self.fileName = glob.glob('/home/rossebv/Documents/RadMAP Tests')
	...
	def indStartClick(self):
		if self.systemClicked.get() == 0 and self.startList != []:		#Prevents from clicking indStart without selecting sensors to start
			self.currentClock = datetime.now()
			if self.operator.get() == "Enter Operator Name Here":
					self.noOperator()
			else:
