...
class operationsApp(Tk):
	...
	def verificationMonitor(self):
		...
		if self.acqStarted:		
			if not self.acqStop:
				if self.troubleshooting:
					print "Verification Owns Sockets"
				time.sleep(1)
				if hsSocket.poll(100) != 0:
					hsVerfMess = hsSocket.recv()
					if self.troubleshooting:
						time2 = time.time()					
				...
				#	bugVerfMess = bugSocket.recv()
				#	if "lidar" in bugVerfMess:
				#		if "True" in bugVerfMess:
						...
						if self.startHyperSpecFlag == 1:
							if self.hyperSpecStarted == 1:
								...
								if hsSocket.poll(100) != 0:
									hsMessage = hsSocket.recv()
									print "%s" % hsMessage
									self.verificationHyperSpec.congfigure(bg="Yellow")
									self.displayVar.set(hsMessage)
									self.communicationDisplay.update()
								time.sleep(9)
								self.hyperSpecStarted = 1
		...
		if self.systemClicked.get() == 1:
			...
			for command in self.stopList:
				if command == "stopHyperSpec":
					...
					time.sleep(4)
					if hsSocket.poll(200) !=0:
						hsStopMess = hsSocket.recv()
						self.displayVar.set(hsStopMess)
				...
				if command == "stopCapture":
					...
					if bugSocket.poll(100) != 0:
						...
						self.communicationDisplay.update()
					time.sleep(10)
					if bugSocket.poll(200) != 0:
						print "Inside Poll, What is on Socket??"
						bugStopMess = bugSocket.recv()
						self.displayVar.set(bugStopMess)
				...
				if command == "stopNeutrons":
					...
					if liqSocket.poll(100) != 0:
						...
						self.communicationDisplay.update()
					time.sleep(90)
					if liqSocket.poll(200) !=0:
						liqStopMess = liqSocket.recv()
						self.displayVar.set(liqStopMess)
...
if __name__ == "__main__":
	...
	app.tick()
	#app.after(1000, app.verificationMonitor())
	app.mainloop()
