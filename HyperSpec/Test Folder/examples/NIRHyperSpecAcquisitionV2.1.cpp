...
void makeCube(std::pair<unsigned short *,int> myData)
{
	...
	char fileDay [100];
	sprintf(fileDay, "%02d%02d%02d", lt.wYear, lt.wMonth, lt.wDay);

	char fileTime [100];
	sprintf(fileTime, "%02d:%02d:%02d.%03d", lt.wHour, lt.wMinute, lt.wSecond, lt.wMilliseconds);

	char directoryTime [100];
	sprintf(directoryTime, "%02d%02d%02d%03d", lt.wHour, lt.wMinute, lt.wSecond, lt.wMilliseconds);
